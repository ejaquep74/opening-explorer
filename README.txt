...
PEND: run from FEN and use list of possible moves. Example for Accelerated Meran I use the position after a6. I can define the possible moves for white as b3, Qc2 and c5. And evaluate only those. This implies running program 3 times, for each move generate different initial FEN. And generate 3 files, you can append the move in the end of filename to distinguish.

PEND: process al example games (importan games and recent games) to evaluate the lines (i.e. 8 moves ahead) and check for interesting thing like sacrifices, forcing moves (only 1-2 good answers). With this build a tree of moves that can be shown in a PGN to the user to get some ideas. Possibly mix this with alternative lines generated by the engine, but only for those cases in which human move is defficient.

PEND: look for lines with threats. Define threat as making a null move and having good score for the other side. If the next move is capture, check if there are multiple recapture and only one works. Also check if the next move is not a recapture, which makes it more interesting. Also check when threat involves no capture in next move (positional threat). Do this for the next 6+ moves so just to check how many threats in the line. You can even explore if more moves by the defending side, still have moves from the attacker that have threats... trying then to build a line with more threats. You can try the moves played by humans only, so not to especulate with some weird engine moves.
