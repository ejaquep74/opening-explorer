...
public class AnnotatedPgnMarkerService {
    ...
    public String markImportantMovesInPgn(String pgnFilePath) throws Exception {
        ...
        for (Game game : pgn.getGames()) {
            ...
            String[] moves = game.getHalfMoves().toString().split("\\s+");
...
//            boolean nextMoveIsImportant = false;
            ...
            for (String moveStr : moves) {
            	...
            	int totalVariations = Optional.ofNullable(gameToVariationsMap.get(moveCounter))
                        .map(variations -> variations.size())
                        .orElse(0);
				log.debug("TOTAL: moveCounter={} totalVariations={}", moveCounter, totalVariations);
            	
            	if (totalVariations > 0 || isImportantMove(moveStr)) {
                    Move move = new Move(moveStr, board.getSideToMove());
                    board.doMove(move);
                    annotatedPgn.append(moveStr)
                                 .append(" { [%csl R")
                                 .append(getDestinationSquare(move))
                                 .append("]} ");
                } else {
                    board.doMove(new Move(moveStr, board.getSideToMove()));
                    annotatedPgn.append(moveStr).append(" ");
                }
            }
            annotatedPgn.append("\n\n");
        }
    ...
    private boolean isImportantMove(String moveStr) {
